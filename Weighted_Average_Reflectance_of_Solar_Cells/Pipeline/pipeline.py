...
# 모델 fitting
@timeit
def fit_models(X_train, y_train, model_name, model, model_path, hyperparams=None, cv=None):
        try:
            ...
            if hyperparams is None:
                ...
                try:
                    opt = BayesSearchCV(model, hyperparams, cv=cv, n_iter=50, scoring='neg_mean_squared_error', n_jobs=-1, random_state = 42)
                    opt.fit(X_train, y_train)
                except:
...
class Solar_pipeline(object):
    ...
    """
    def __init__(self, dataset, target, hyperparams_path, model_path, pred_path, cv = 10, test_size=0.3, random_state=42):
        ...
        for model_name, model in zip(['LinearRegression', 'Ridge', 'Lasso', 'ElasticNet', 'BayesianRidge', 'ARDRegression',
                                      ...
                                      'MLPRegressor'],
                                        [LinearRegression(), Ridge(), Lasso(), ElasticNet(), BayesianRidge(), ARDRegression(),
                                            SGDRegressor(), SVR(), RandomForestRegressor(), GradientBoostingRegressor(), AdaBoostRegressor(),
                                            XGBRegressor(), LGBMRegressor(), CatBoostRegressor(verbose=False), KNeighborsRegressor(), DecisionTreeRegressor(),
                                            MLPRegressor()]):
    
            self.best_models[model_name] = fit_models(self.X_train, self.y_train, model_name, model, self.model_path, hyperparams=self.hyperparams[model_name], cv=self.cv)
            try:
                print("BEST PARAMS >> " + model_name + ": " + str(self.best_models[model_name].best_params_))
            except:
            ...
            self.perf_table = update_table(self.perf_table, y_pred, self.y_test, model_name)
    ...
    def summarize(self, metric = 'RMSE'):
        ...
        print('Best performance: ' + str(self.perf_table.sort_values(by=metric, ascending=True).iloc[0]))
        
        Best_model = load_model(self.model_path + Best_model + '.pkl')
        y_pred = Best_model.predict(self.X_test)
        
        # 그래프 그리기
        sns.scatterplot(x=self.y_test, y=y_pred, alpha=0.4)
        sns.regplot(x=self.y_test, y=y_pred, scatter_kws={'s': 20, 'alpha':0.3}, line_kws={'color':'green', 'linewidth': 2}, robust=True)
        ...
        test_pred = pd.DataFrame({'Actual Thickness': self.y_test, 'Predicted Thickness': y_pred})
        
        return Best_model, y_pred, test_pred
